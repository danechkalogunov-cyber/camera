...
* The **only** nonisolated entry point is `VideoRenderer.enqueue(_:)` (§3.2), which is called from
...
static inline float2 bobFieldUV(float2 uv, float parity, float2 texel) {
    ...
    // Return the interpolation endpoints packed as (vA, vB) plus fraction via a second call site.
...
static inline float cornerCoverage(float2 fragPx, constant TileUniforms &u) {
    const float2 half = u.viewportSizePx * 0.5;
    const float2 p = fragPx - half;                      // centre-origin, y down
    float r;
...
vertex VSOut vigilMaskVertex(uint vid [[vertex_id]], constant MaskUniforms &m [[buffer(0)]]) {
    const float2 p = kQuad[vid];
    const float2 t = float2(p.x * 0.5 + 0.5, 0.5 - p.y * 0.5);
    VSOut o;
    o.position = float4(mix(m.rectNDC.xy, m.rectNDC.zw, float2(t.x, 1.0 - t.y)), 0.0, 1.0);
    o.uv = mix(m.texRect.xy, m.texRect.zw, t);
    return o;
}
