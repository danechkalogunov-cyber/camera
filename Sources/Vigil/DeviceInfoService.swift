...
final class DeviceInfoService {
    ...
    //
    // ⚠️ These three are `internal`, not `private`, and only because `DeviceInfoService+Image.swift`
    // drives them: `private` reaches a type's extensions within one file only. They are the writer's
    // own state and nothing else in this target may touch them.
    ...
    var pendingImage: InspectorImageSettings?
    ...
    var confirmedImage: InspectorImageSettings?
    ...
    var imageWriter: Task<Void, Never>?
