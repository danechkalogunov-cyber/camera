    ...
    extension MainWindowView {
        ...
        func connectStageCell(_ id: CameraID) {
            ...
            Task {
                ...
                // ⚠️ No number in the sentence, and that is the change `F-DEC-06` forces. The limit
                // used to be four cameras and could be written out — Russian declines it, so a
                // formatted `%lld` would have needed a `.stringsdict` for a constant. It is now a
                // decode budget: what it admits depends on the machine and on what these particular
                // cameras cost, so there is no number to name and claiming one would be a lie on
                // most machines.
                window.toast = MainWindowToast(
                    kind: .warning,
