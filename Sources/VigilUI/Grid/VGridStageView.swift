...
package struct VGridStageView<Video: View>: View {
    ...
    package init(assignment: VStageAssignment,
        ...
        self.cameraIndex = Dictionary(cameras.map { ($0.id, $0) }, uniquingKeysWith: { first, _ in
            first
        })
        self.video = video
    }
    ...
    package var body: some View {
        ...
        .onKeyPress(keys: [.leftArrow, .rightArrow, .upArrow, .downArrow, .return, .space, .delete],
    ...
    private func cell(_ slot: VStageSlot, in geometry: VGridGeometry) -> some View {
        ...
        if let camera = slot.camera, let model = cameraIndex[camera], !model.isStreaming {
            ...
            VGridEmptyCell(position: slot.index + 1,
    ...
    private func tile(_ model: VStageCamera, slot: VStageSlot) -> some View {
        VGridTileView(camera: model.camera,
                      state: model.state,
...
private struct VStageGeometryMatch: ViewModifier {
    ...
    func body(content: Content) -> some View {
        if let namespace {
            content.matchedGeometryEffect(id: camera.rawValue.uuidString,
                                          in: namespace,
                                          properties: .frame,
                                          anchor: .center,
                                          isSource: isSource)
...
private struct GridStagePreviewHost: View {
    ...
    private static func makeCameras(_ count: Int) -> [VStageCamera] {
        (0..<count).map { index in
            ...
            return VStageCamera(camera: identity,
                                ...
                                stats: VTileStats(codec: index % 2 == 0 ? "H.265" : "H.264",
                                                  dimensions: FrameDimensions(width: 1920,
                                                                              height: 1080),
                                                  framesPerSecond: 25,
    ...
    var body: some View {
        VGridStageView(assignment: assignment,
                       mode: mode,
                       cameras: cameras,
                       selection: cameras.first?.id,
                       focusedIndex: focusedIndex) { camera in
